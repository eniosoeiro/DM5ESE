...
final class ThicknessSyncRules {
    ...
    static final class Failure extends IOException {
        final boolean retryable; final int httpStatus;
        Failure(String message, boolean retryable) { this(message,retryable,0); }
        Failure(String message, boolean retryable, int httpStatus) { super(message); this.retryable=retryable; this.httpStatus=httpStatus; }
    }
    ...
    static Failure httpFailure(int code) {
        if (code == 401 || code == 403) return new Failure("Sessão expirada ou conta sem acesso. Entre novamente; os arquivos foram preservados.", false, code);
        ...
        return new Failure("O servidor recusou o envio (HTTP " + code + "). Confira a configuração e o formato da captura.", false, code);
    }
